...
module.exports = function(options) {
  ...
  options.resize || (options.resize = '160x160');
    ...
    if (regex.test(req._parsedUrl.pathname)) {
      ...
      src = (function(src) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          file = _ref[_i];
          if (!/^(\.DS.+|Network Trash Folder|Temporary Items|\.Apple.*|Thumbs.db)$/i.test(file)) {
      ...
      return async.series([
        function(done) {
          ...
          return exec("convert -define jpeg:size=" + options.resize + " -resize " + options.resize + " '" + tmp + "' '" + dst + "'", function(err) {
            if (err) {
              console.error(err);
            }
